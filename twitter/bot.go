...
func PostTweet(ctx context.Context, message string, imageBytes []byte) (string, error) {
    ...
    if imageBytes != nil {
        ...
        // <<< 2. FIX: Base64-encode the image bytes before uploading.
        base64Image := base64.StdEncoding.EncodeToString(imageBytes)
        media, err := api.UploadMedia(base64Image) // Pass the encoded string
    ...
    // Use the dedicated OAuth1 client to send the request.
